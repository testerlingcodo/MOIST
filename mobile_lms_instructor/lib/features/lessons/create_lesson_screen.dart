  ...
  Future<void> _save() async {
    ...
    if ((_selectedSubjectId ?? '').isEmpty || title.isEmpty || url.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Subject, title, and video URL are required.')),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lesson posted.')),
      );
      if (context.canPop()) {
        context.pop();
      }
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to post lesson: $e')),
      );
    } finally {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(_loading ? 'Posting...' : 'Post Lesson'),
                  ),
                ),
              ],
            ),
